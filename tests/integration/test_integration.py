...
class BaseTestCases:
    class BaseTest(unittest.TestCase):
        ...
        def __init__(self, *args, optimization_level: int, **kwargs):
            ...
            for name, cases in test_cases.items():
                with self.subTest():
                    self.run_test(
                        ...
                        self.create_create_verilog_from_func(threshold=0),
                    )
                    logging.info(
                        f"Testing {self.__class__.__name__} on {name} with {cases}"
                    )
            df = pd.DataFrame(
...
class Opti0(BaseTestCases.BaseTest):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, optimization_level=0, **kwargs)


@pytest.mark.usefixtures("argparse")
class Opti1(BaseTestCases.BaseTest):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, optimization_level=1, **kwargs)


@pytest.mark.usefixtures("argparse")
class Opti2(BaseTestCases.BaseTest):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, optimization_level=2, **kwargs)


@pytest.mark.usefixtures("argparse")
class Opti2(BaseTestCases.BaseTest):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, optimization_level=3, **kwargs)
