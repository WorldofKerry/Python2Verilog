...
def generic_run_test(
    test_class: unittest.TestCase,
    function_name: str,
    test_cases: list[tuple],
    args: dict,
    dir: str,
    create_verilog_from_func: Callable[[ast.FunctionDef], tuple[CodeGen, ir.Vertex]],
):
    """
    ...
    FILES_IN_ABS_DIR = {
        key: os.path.join(ABS_DIR, value) for key, value in config["file_names"].items()
    }
    fifos = {
    ...
    with open(FILES_IN_ABS_DIR["python"]) as python_file:
        ...
        statistics["module_nchars"] = len(module_str.replace("\n", "").replace(" ", ""))
        ...
        test_class.all_statistics.append(statistics)
