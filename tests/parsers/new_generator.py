...
def script(args: argparse.Namespace, logger: logging.Logger, shell: callable) -> int:
    ...
    with open(os.path.join(PATH_TO_NEW_TEST, args.config), mode="w") as config_file:
        ...
        logger.error(f"{PATH_TO_NEW_TEST}")
        config["file_names"] = {
            ...
            "iverilog_cmd": f"iverilog -s generator_tb {os.path.join(PATH_TO_NEW_TEST, args.module)} {os.path.join(PATH_TO_NEW_TEST, args.testbench)} && unbuffer vvp a.out > {os.path.join(PATH_TO_NEW_TEST, args.actual)} && cat {os.path.join(PATH_TO_NEW_TEST, args.actual)}\n",
        }
    ...
    with open(
        ...
        sim_cmds_file.write(
            f"iverilog -s generator_tb {os.path.join(PATH_TO_NEW_TEST, args.module)} {os.path.join(PATH_TO_NEW_TEST, args.testbench)} && unbuffer vvp a.out > {os.path.join(PATH_TO_NEW_TEST, args.actual)} && cat {os.path.join(PATH_TO_NEW_TEST, args.actual)}\n"
        )
